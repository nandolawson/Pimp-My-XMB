...
{
	if (exists((appfolder+"/terms-accepted.cfg").c_str())!=0)//terms not yet accepted
	{
		...
		Mess.Dialog(MSG_YESNO,"You are solely responsible for complying with the applicable laws in your country and you must cease using this software should your actions during the \"software\" operation lead to or may lead to infringement or violation of the rights of the respective content copyright holders.\n\nDo you accept this terms?");
		if (Mess.GetResponse(MSG_DIALOG_BTN_YES)==1)
		{
			create_file((appfolder+"/terms-accepted.cfg").c_str());
			return 0;
		}
		else return -1;
	}
...
string copy_prepare(string appfolder, string operation, string fw_folder, string app)
{
	...
	if (operation=="backup")
	{
		check_path=appfolder+"/content/"+app+"/"+fw_folder;
		...
		while ( (dirp = readdir(dp) ) )
		{
			if ( strcmp(dirp->d_name, ".") != 0 && strcmp(dirp->d_name, "..") != 0 && strcmp(dirp->d_name, "") != 0)
			{
				if (dirp->d_type == DT_DIR)
				{
					...
					dest_paths[findex]=appfolder+"/backup/"+dirp->d_name;
